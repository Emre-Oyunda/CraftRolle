...
if ($currentUser) {
    header('Location: ' . base_url_local('dashboard.php'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mb_strlen($form['username']) < 3) {
        ...
        try {
            ...
            $check->execute([$form['username'], $form['email']]);
            if ($existing = $check->fetch(PDO::FETCH_ASSOC)) {
                $error = 'Bu kullanıcı adı ya da e-posta zaten kullanımda.';
            } else {
                ...
                if ($uid <= 0) {
                    $uid = (int)$pdo->query('SELECT LAST_INSERT_ID()')->fetchColumn();
                }

                $_SESSION['user_id'] = $uid;
                track_user_session_login($uid);

                header('Location: ' . base_url_local('dashboard.php'));
                exit;
            }
        } catch (Throwable $th) {
            $msg = trim((string)$th->getMessage());
...
<title><?= e(APP_NAME) ?> • Kayıt Ol</title>
...
body.auth-page.dark-theme .bullet-grid {
...
<div class="page-shell">
  <header class="top-bar glass-panel">
    ...
    <div class="top-controls">
      ...
      <nav class="top-nav">
        <a class="nav-pill" href="<?= base_url_local('login.php') ?>">Giriş</a>
        <a class="nav-pill is-active" href="<?= base_url_local('register.php') ?>">Kayıt Ol</a>
      </nav>
    </div>
  </header>
  ...
  <main class="auth-grid">
    <section class="hero-card glass-panel">
      <h1>Kendi evrenine hoş geldin ✨</h1>
      <p>CraftRolle seni hikâyelerini özgürce inşa etmeye davet ediyor. Renkli kapaklardan detaylı haritalara, karakter notlarından 3D kitaplara kadar her şey tek bir platformda.</p>
      <ul class="bullet-grid">
        <li>🚀 Fikirden kitaba ışık hızında</li>
        <li>🎨 Kapak & harita stüdyoları</li>
        <li>🧠 Karakter ve sahne arşivi</li>
        <li>🤝 Paylaş, geri bildirim al, büyü</li>
      </ul>
      <a class="hero-link" href="<?= base_url_local('login.php') ?>">Zaten hesabın var mı? Giriş yap →</a>
    </section>

    <section class="auth-card glass-panel">
      <h2>Kayıt Ol</h2>
      <p class="hint">Bir dakikadan kısa sürede hesabını oluştur ve hikâyeni büyütmeye başla.</p>
      <?php if ($error): ?>
      ...
      <form class="auth-form" method="post" action="" novalidate>
        ...
        <label class="field">
          ...
          <input class="field-input" type="text" name="username" value="<?= e($form['username']) ?>" autocomplete="username" required>
        </label>

        <label class="field">
          <span class="field-label">E-posta</span>
          <input class="field-input" type="email" name="email" value="<?= e($form['email']) ?>" autocomplete="email" required>
        </label>
        ...
        <label class="field">
          ...
          <input class="field-input" type="password" name="password_confirm" autocomplete="new-password" required>
        </label>

        <button class="primary-btn" type="submit">CraftRolle&apos;a Katıl</button>
      </form>
      <div class="auth-alt">
        <span>Hesabın var mı?</span>
        <a href="<?= base_url_local('login.php') ?>">Giriş yap</a>
      </div>
    </section>
  </main>
  ...
  <footer class="page-footer">© <?= date('Y') ?> <?= e(APP_NAME) ?> • Hayal et, yaz, paylaş</footer>
</div>
